...
def broadcast_transaction(user_id):
    ...
    for node_id in user.get_known_nodes():
        if node_id != user_id:  # Skip broadcasting to the user who initiated the transaction
            node = users.get(node_id)
            node.blockchain.create_transaction(data['sender'], data['recipient'], data['amount'], user.public_key)
